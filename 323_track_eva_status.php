...
<html>
	<head>
		...
		<title>Watch_Dog | Track Evaluate Status</title>
											...
											<div class="row">
												<div class="col-xs-12">
													...
													<div class="progress-group">
														<span class="progress-text">ยอดผู้ทำแบบประเมิน</span>
														...
														<strong>ผลการประเมิน</strong></p>
															<table class="table table-striped">
																<TR>
																	...
																	<TH>ต่ำสุด</TH>
																	<TH>เฉลี่ย</TH>
																	<TH>เบี่ยงเบนมาตรฐาน</TH>
																</TR>
																<Tbody id="tbl_eva_result">
																</Tbody>
												...
												<div class="col-xs-12">
													<div class="box box-success">
														<div class="box-header">
														  ...
														  <h3 class="box-title">ความคิดเห็นและข้อเสนอแนะ</h3>
														  </div>
														<div class="box-body chat" id="comment_box">
														</div>
													  </div>
													  <!-- /.box (chat box) -->
												</div>
...
function get_table_data()
{
	...
	.done(function(data) {
		...
		jQuery.each( data_arr, function( i, val ) {
			...
			{
				print_text +="<TD><font color='#ff8080'><B>ยังไม่ได้ทำประเมิน</B></font></TD>";
				print_text +="<TD></TD>";
			}
			else
			{
				Check_in_CNT = Check_in_CNT + 1;
				print_text +="<TD><font color='#00cc00'><B>ทำประเมินแล้ว</B></font></TD>";
...
function load_Eva_result()
{
	...
	.done(function(data) {
		...
		if (data != "[]")
		{
			...
			jQuery.each( data_arr, function( i, val ) {
				print_text += "<TR>";
				print_text += "<TD>"+val.item_data+"</TD>";
				print_text += "<TD>"+val.max+"</TD>";
				print_text += "<TD>"+val.min+"</TD>";
				print_text += "<TD>"+val.average+"</TD>";
				print_text += "<TD>"+val.SD+"</TD>";
				print_text += "</TR>";
			});
			//alert (print_text)
			$("#tbl_eva_result").html(print_text)
		}
	})
	.fail(function() {
		// just in case posting your form failed
...
function load_eva_comment()
{
	...
	.done(function(data) {
		...
		if (data != "[]")
		{
			...
			print_text = "";
			jQuery.each( data_arr, function( i, val ) {
				print_text += '<div class="item">';
				print_text += '<img src="img/wd_img/'+val.wd_img+'" alt="user image" class="offline">';
				print_text += '<p class="message">';
				print_text +=  '<a href="#" class="name">';
				print_text +=    val.n_name + "<small class='text-muted'> รุ่น "+val.gen+"</small>";
				print_text +=  '</a>';
				print_text +=   val.msg
				print_text +=  '</p>';
				print_text +=  '</div>';
			});
			$("#comment_box").html(print_text)
		}
	})
	.fail(function() {
		// just in case posting your form failed
