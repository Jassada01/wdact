...
{	//Var
	...
	<table border="1" >
		<tr>
			<th align="center">ชนิดสินค้า<Br>Type</th>
			<th align="center">จำนวนในระบบ<Br>System</th>
			<th align="center">จำนวนที่นับจริง<Br>Actual</th>
			<th align="center">หมายเหตุ<Br>Remark</th>
		</tr>
		<tr>
			<td align="center"></td>
	...
	$pdf->Text($iw + 5, $ih + 22, 'วันที่ (Date) : '.$date) ;
	$pdf->Text($iw + 65, $ih + 22, 'ตำแหน่ง (Location) : '.$wh) ;
	$pdf->Text($iw + 110, $ih + 22, 'ลำดับที่ (No.)  ') ;
	
	$pdf->Text($iw + 8, $ih + 62, 'RM (Full size)') ;
	$pdf->Text($iw + 8, $ih + 69, 'PM (Partial size)') ;
	$pdf->Text($iw + 8, $ih + 76, 'รวม(Total)') ;
	
	$pdf->SetFont('thsarabunb', '', 16, '', true);
	$pdf->Text($iw + 120, $ih + 28, $cnt) ;
	$pdf->Text($iw + 38, $ih + 28, $item_c);
	$pdf->Text($iw + 38, $ih + 34, $item_n) ;
	
	
	$pdf->SetFont('thsarabun', '', 16, '', true);
	$pdf->Text($iw + 5, $ih + 28, 'รายละเอียด (Item)  : ') ;
	...
	$pdf->Text($iw + 5, $ih + 88, 'นับโดย______________________') ;
	$pdf->Text($iw + 55, $ih + 88, 'ตรวจโดย______________________') ;
	$pdf->Text($iw + 105, $ih + 88, 'วันที่_____________') ;
	$pdf->Text($iw + 5, $ih + 84, 'Count By') ;
	$pdf->Text($iw + 55, $ih + 84, 'Check By') ;
	$pdf->Text($iw + 105, $ih + 84, 'Date') ;
